...
def init_plugin():
    #Smallest dv first, time treshold
    config = {'plugin_name':'MVPSFTT',
     'plugin_type':'sim'}
    return (
     config, {})

class MVPSFTT(ConflictResolution):
    def __init__(self):
        super().__init__()
    ...
    def resolve(self, conf, ownship, intruder):
        ...
        #Start and end of time treshold for time to los
        tstart =200
        tend = 100
        ...
        wtLOS_all = np.zeros((nac,nac)) #Weight due to tlos
        ...
        for ((ac1, ac2), qdr, dist, tcpa, tLOS) in zip(conf.confpairs, conf.qdr, conf.dist, conf.tcpa, conf.tLOS):
            ...
            if idx1 >-1 and idx2 > -1:
                ...
                if tLOS<tstart:
                    wtLOS_all[idx1, idx2]  = np.maximum(0,np.minimum((tstart-tLOS)/tend,1))
        ...
        s1 = np.where(sides < 0, 1, 0) #np.where(sides < 0, True, False)
        s2 = np.ones(s1.shape)-s1 #  ~s1
        ...
        for i in range(len(dv_abs)):
            if dv_abs1[i] < dv_abs2[i]:
                wsum = np.maximum(0,np.minimum(wtLOS_all[i]+s1[i],1))
                dv_mvp_all[i] = ((dv_mvp_all[i].T * wsum).T)/np.sum(wsum)
            elif dv_abs1[i] > dv_abs2[i]:
                wsum = np.maximum(0, np.minimum(wtLOS_all[i] + s2[i],1))
                dv_mvp_all[i] = ((dv_mvp_all[i].T * wsum).T)/np.sum(wsum)
            else:
