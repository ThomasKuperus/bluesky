...
def init_plugin():
    config = {'plugin_name':'MVPAvgSSO',
     'plugin_type':'sim'}
    return (
     config, {})

class MVPAvgSSO(ConflictResolution):
    #The average same side only, so if all resolution vectors are in the same direction
    def __init__(self):
        super().__init__()
    ...
    def resolve(self, conf, ownship, intruder):
        ...
        for i in range(len(dv_abs)):
            if all(s1[i])==True or all(s2[i])==True:
                dv_mvp_all[i] = (dv_mvp_all[i] / conf_count[i])  # take the average instead of sum

        dv_mvp_all = np.nan_to_num(dv_mvp_all, 0)
