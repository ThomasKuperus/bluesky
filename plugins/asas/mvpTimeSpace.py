...
class MVPTimeSpace(ConflictResolution):
    ...
    def resolve(self, conf, ownship, intruder):
        ...
        # The program loops through the resolution for each conflict seperately
        ...
        for ((ac1, ac2), qdr) in zip(conf.confpairs, conf.qdr):
            ...
            conf_count[idx1] += 1
            #Determine on which side of ownship the intruder is.
            if ownship.hdg[idx1] < 180:
                ...
                if -180<(qdr - ownship.hdg[idx1])<=0:
                    side1[idx1] += 1
                else:
                    side2[idx1] += 1

        #Store how many aircraft are on each side
        #side1 = np.zeros((ownship.ntraf, 1))
        #side2 = np.zeros((ownship.ntraf, 1))
        #ownships = list(list(zip(*conf.confpairs))[0])
        #side = ownship.hdg[[ownship.id.index(i) for i in list(list(zip(*conf.confpairs))[0])]] - conf.qdr
        ...
        for ((ac1, ac2), qdr, dist, tcpa, tLOS) in zip(conf.confpairs, conf.qdr, conf.dist, conf.tcpa, conf.tLOS):
            ...
            if idx1 >-1 and idx2 > -1:
                ...
                #works in this example, but it might very well be coincidence.
                #Weighting based on free space is interesting, but which vectors to weight?
                #the vectors from the open side might push towards the crowd, and the other way around
                #if ownship.hdg[idx1] < 180:
                #    if 0 < (qdr - ownship.hdg[idx1]) <= 180:
                #        spaceWeight = 1/(side1[idx1]/(conf_count[idx1]/2))
                #        #spaceWeight =side2[idx1] / (conf_count[idx1] / 2)
                #    else:
                #        spaceWeight = 1/(side2[idx1]/(conf_count[idx1]/2))
                #        #spaceWeight =side1[idx1] / (conf_count[idx1] / 2)
                #else:
                #    if -180 < (qdr - ownship.hdg[idx1]) <= 0:
                #        spaceWeight = 1/(side1[idx1]/(conf_count[idx1]/2))
                #        #spaceWeight =side2[idx1] / (conf_count[idx1] / 2)
                ...
                if self.swprio:
                    ...
                    #print(ac1, ac2, dv_mvp, hdg_dv, ownship.hdg)
                    if ownship.hdg[idx1] < 180:
                        if ownship.hdg[idx1] < hdg_dv <= (ownship.hdg[idx1]+180):
                            spaceWeight = 1 / ((side1[idx1]+1) / ((conf_count[idx1]+2) / 2))
                            #spaceWeight =side2[idx1] / (conf_count[idx1] / 2)
                            #spaceWeight = 1 - ( side1[idx1] -side2[idx1]) / (conf_count[idx1] / 2)
                        else:
                            spaceWeight = 1 / ((side2[idx1]+1) / ((conf_count[idx1]+2) / 2))
                            #spaceWeight =side1[idx1] / (conf_count[idx1] / 2)
                            #spaceWeight = 1 - (side2[idx1] - side1[idx1]) / (conf_count[idx1] / 2)
                    else:
                        if (ownship.hdg[idx1]-180) < hdg_dv <= ownship.hdg[idx1]:
                            ...
                            #spaceWeight = 1 - (side2[idx1] - side1[idx1]) / (conf_count[idx1] / 2)
                    #print(idx1)
                    #print('space',spaceWeight)
                    #print((self.dtlookahead - tLOS))
                    dv[idx1] = dv[idx1] - dv_mvp*(self.dtlookahead - tLOS)*spaceWeight
                    #print(ac1, ac2, spaceWeight,hdg_dv,side1[idx1], side2[idx1],conf_count[idx1],ownship.hdg[idx1],qdr)
        ...
        #print('timespace')
        #print(list(list(zip(*conf.confpairs))[0]))
        #print(dv)
        newv = v + dv
        #print(newv)
