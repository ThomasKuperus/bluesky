...
from bluesky.traffic.performance.openap import perfoap
...
class MVP2(ConflictResolution):
    ...
    def resolve(self, conf, ownship, intruder):
        ...
        for ((ac1, ac2), qdr, dist, tcpa, tLOS) in zip(conf.confpairs, conf.qdr, conf.dist, conf.tcpa, conf.tLOS):
            ...
            # TK The resolution is applied to each conflict separtely and solved to the index of ownship,
            # so conf.confpairs probably contains each conflict twice, once with each aircraft as ownship
            # An possibility to add weights, could be based on relative importance in terms of time to LOS
            # Could be done by keeping track of t to los  and number of conflicts in a separate array, like dv,
            # Then multiply each dv with t and at the end divide it by the average t to los
            if idx1 >-1 and idx2 > -1:
                ...
                if self.swprio:
                    ...
                    dv[idx1] = dv[idx1] - dv_mvp*(self.dtlookahead - tLOS)
                    #print(ac1, ac2, dv_mvp, dist, (self.dtlookahead - tLOS),conf_time[idx1]/conf_count[idx1])
        ...
        #print('dv:',dv,type(dv))
        #print('ownship.tas',ownship.tas)
        #print('dv_wp:',dv_wp,type(dv_wp))
        ...
        # Cap the velocity
        newgscapped = np.maximum(vcas2tas(ownship.perf.vmin,ownship.alt),np.minimum(vcas2tas(ownship.perf.vmax,ownship.alt),newgs)) #It seems like vmax here is max cas in m/s (based on what is seen in bs), where newgs is gs in m/s
        #print('dv:',dv)
        #print('v:',v)
        #print('newv:',newv)
        #print('newgs:', newgs)v
        #vmin = ownship.perf.vmin[0]
        #vmax = ownship.perf.vmax[0]
        #vminkts = vmin*kts
        #vmaxkts = vmax*kts
        #alt = ownship.alt[0]
        #print('CAS vmin, vmax:',ownship.perf.currentlimits(0)[0],ownship.perf.currentlimits(0)[1],vcas2mach(ownship.perf.currentlimits(0)[0],alt), vcas2mach(ownship.perf.currentlimits(0)[1],alt))
        #print("alt: ",alt)
        #print('CAS,TAS,M: vmin, vmax:',vmin,vmax,vcas2tas(vmin,alt), vcas2tas(vmax,alt),vcas2mach(vmin,alt), vcas2mach(vmax,alt))
        #print('vmin, vmax:', vmin,vmax, vtas2mach(vmin,alt), vtas2mach(vmax,alt))
        #print('kts')
        #print('CAS,TAS,M: vmin, vmax:',vmin,vmax,vcas2tas(vminkts,alt), vcas2tas(vmaxkts,alt),vcas2mach(vminkts,alt), vcas2mach(vmaxkts,alt))
        #print('vmin, vmax:', vminkts,vmaxkts, vtas2mach(vminkts,alt), vtas2mach(vmaxkts,alt))
        #print('allow_v_tas:',ownship.perf)
        #print('newgscapped:',newgscapped)
        #print('vlim:', newgs-newgscapped)
