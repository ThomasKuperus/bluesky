...
def init_plugin():
    config = {'plugin_name':'MVPDistFTR',
     'plugin_type':'sim'}
    return (
     config, {})

class MVPDistFTR(MVP):
    ...
    def resolve(self, conf, ownship, intruder):
        ...
        for ((ac1, ac2), qdr, dist, tcpa, tLOS, nolook) in zip(conf.confpairs, conf.qdr, conf.dist, conf.tcpa,
                                                               conf.tLOS, conf.nolook):
