...
def init_plugin():
    config = {'plugin_name':'MVPAngleW',
     'plugin_type':'sim'}
    return (
     config, {})

class MVPAngleW(ConflictResolution):
    def __init__(self):
        super().__init__()
    ...
    def resolve(self, conf, ownship, intruder):
        ...
        dv = np.zeros((ownship.ntraf, 3,sol_mat_depth)) #2D solution, only horizontal
        conf_count = np.zeros((ownship.ntraf, sol_mat_depth))
        weight = np.zeros((ownship.ntraf, sol_mat_depth))
        dv_hdg = np.zeros((ownship.ntraf, sol_mat_depth))
        dv_mag = np.zeros((ownship.ntraf, sol_mat_depth))
        ...
        for ((ac1, ac2), qdr, dist, tcpa, tLOS) in zip(conf.confpairs, conf.qdr, conf.dist, conf.tcpa, conf.tLOS):
            ...
            if idx1 >-1 and idx2 > -1:
                ...
                if conf_count[idx1]>sol_mat_depth:
                    ...
                    dv_hdg = np.hstack([dv_hdg,np.zeros((ownship.ntraf,1))])
                    dv_mag = np.hstack([dv_mag, np.zeros((ownship.ntraf, 1))])
                    dv = np.dstack([dv,np.zeros((ownship.ntraf, 3,1))])

                dv_mvp, tsolV = self.MVP(ownship, intruder, conf, qdr, dist, tcpa, tLOS, idx1, idx2)
                if tsolV < timesolveV[idx1]:
                    timesolveV[idx1] = tsolV
                #dv_mvp_hdg = (np.arctan2(dv_mvp[0],dv_mvp[1])*180/np.pi) %360
                #dv_mvp_mag = dv_mvp[0]*dv_mvp[0]+dv_mvp[1]*dv_mvp[1]
                #d_hdg = np.arccos(np.dot(dv_mvp,dv[idx1,0:2,0:conf_count[idx1]])/(dv_mag[idx1,0:conf_count[idx1]]*dv_mvp_mag))

                #Store results
                dv_mag[idx1,int(conf_count[idx1]-1)] = np.sqrt(dv_mvp[0]*dv_mvp[0]+dv_mvp[1]*dv_mvp[1])
                #weight[idx1] = weight[idx1]+ np.sum(abs(d_hdg)) #difference between headings, compensate for
                dv[idx1,:,int(conf_count[idx1]-1)] = -dv_mvp

                #keeping track of number of conflicts


        #Calculate weights
        for own in range(ownship.ntraf):
            for conflict in range(int(conf_count[own])):
                floating_error_margin = 0.0000001
                d_hdg = np.arccos(np.dot(dv[own,0:2,conflict],dv[own,0:2,0:int(conf_count[own])])/(dv_mag[own,conflict]*dv_mag[own,0:int(conf_count[own])]+floating_error_margin))
                weight[conflict] = np.sum(abs(d_hdg)/np.pi)/(conf_count[own])+1/(conf_count[own])

            if int(conf_count[own])>0:
                dv[own, :, :] = dv[own, :, :] * weight

        dv_sum =np.sum(dv,axis=2)
        dv = dv_sum
