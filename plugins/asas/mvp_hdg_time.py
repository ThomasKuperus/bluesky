...
class MVP_hdg_time(ConflictResolution):
    def __init__(self):
        super().__init__()
    ...
    def resolve(self, conf, ownship, intruder):
        ...
        #Weight based on next waypoint
        #First find next active waypoint of all aircraft
        wpLat = bs.traf.actwp.lat[:]
        wpLon = bs.traf.actwp.lon[:]
        #heading to waypoint
        hdg_wp, dist_wp = geo.kwikqdrdist(ownship.lat, ownship.lon,
                                    wpLat, wpLon)

        #velocity vector to waypoint, this vector is not calculated right
        dv_wp = np.transpose(np.array([ownship.tas*np.cos(hdg_wp),ownship.tas*np.sin(hdg_wp),ownship.tas*0]))
        ...
        dv_sol = np.divide(dv, conf_time_avg , out=np.zeros_like(dv), where=conf_time_avg  != 0)
        dv = (dv_sol*conf_count + dv_wp)/(conf_count+1)
        dv = np.transpose(dv)
        ...
        newv = v + dv
        print('dv_sol:',dv_sol,type(dv_sol))
    ...
    #    dv3 = np.maximum(mindv3,np.minimum(maxdv3,dv3))
        ...
        dv = np.array([dv1,dv2,dv3]) #East, North, Vertical
