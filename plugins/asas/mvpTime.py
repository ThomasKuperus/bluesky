...
def init_plugin():
    config = {'plugin_name':'MVPTime',
     'plugin_type':'sim'}
    return (
     config, {})

class MVPTime(ConflictResolution):
            ...
            if idx1 > -1:
                if idx2 > -1:
                    ...
                    if self.swprio:
                        ...
                        dv[idx1] = dv[idx1] - dv_mvp * (1 / tLOS)
                    if self.noresoac[idx2]:
                        dv[idx1] = dv[idx1] + dv_mvp
                if self.resooffac[idx1]:
                    dv[idx1] = 0.0

        tLOS_avg = np.divide(tLOS_all, conf_count, out=(np.zeros_like(tLOS_all)), where=(conf_count != 0))
        #print(dv)
        dv = np.divide(dv, tLOS_avg, out=(np.zeros_like(dv)), where=(tLOS_avg != 0))
        #print(dv)
        dv = np.transpose(dv)
