...
def append_commands(newcommands):
    ...
    for cmd, (smallhelp, args, fun, largehelp) in newcommands.items():
        ...
        while args:
            opt = args[0] == "["
            cut = (
                args.find("]") if opt else args.find("[") if "[" in args else len(args)
            )
...
class Argparser:
    # Global variables
    reflat = -999.0  # Reference latitude for searching in nav db
    # in case of duplicate names
    reflon = -999.0  # Reference longitude for searching in nav db
    # in case of duplicate names
            ...
            for i, argtypei in enumerate(argtype):
                    ...
                    if not isinstance(result[0], np.ndarray):
                        if None in result:
                            if not self.argisopt[curtype]:
                                self.error = (
                                    "No value given for mandatory argument "
                ...
                # No success yet with this type (maybe we can try other ones)
                    ...
                    if i < len(argtype) - 1:
                        ...
                        self.error = (
                            "Syntax error processing argument "
                            + str(curtype + 1)
                            + ":\n"
                            + self.error
                        )
    ...
    def parse_arg(self, argtype):
        ...
        # Empty arg or wildcard
            ...
            # lat/lon argument type we also need to it up:
                ...
                if success:
                    ...
                    self.error += (
                        posobj
                    )  # contains error message if txt2pos was no success
                    return False
        ...
        # CAS[kts] Mach: convert kts to m/s for values=>1.0 (meaning CAS)
            ...
            try:
                spd = float(
                    curargu.replace("M0.", ".").replace("M", ".").replace("..", ".")
                )
